...
namespace Lux.Indicators.DivergenceDetectors
{
    ...
    /// 移动平均线背离分析器
    /// </summary>
    public static class MovingAverageDivergenceAnalyzer
    {
        /// <summary>
        /// 查找移动平均线背离点
        /// </summary>
        /// <param name="closePrices">收盘价序列</param>
        /// <param name="movingAverageOutputs">移动平均线分析结果序列</param>
        /// <param name="indicatorSelector">选择用于背离检测的指标值的函数，默认使用短期均线</param>
        ...
        public static List<DivergenceCommon.DivergencePoint> FindDivergences(
            ...
            List<MovingAverageOutput> movingAverageOutputs,
            Func<MovingAverageOutput, decimal> indicatorSelector = null,
            int lookbackPeriod = 10,
            decimal threshold = 0.1m)
        {
            // 默认使用短期均线进行背离检测
            indicatorSelector ??= output => output.ShortMa;
            
            var divergences = new List<DivergenceCommon.DivergencePoint>();
            
            if (closePrices == null || movingAverageOutputs == null || 
                closePrices.Count != movingAverageOutputs.Count || closePrices.Count < lookbackPeriod)
            {
                return divergences;
            }
            ...
            var indicatorPeaks = DivergenceCommon.FindLocalExtrema(movingAverageOutputs.Select(indicatorSelector).ToList(), lookbackPeriod);
            ...
            foreach (var pricePeak in pricePeaks)
            {
                foreach (var indicatorPeak in indicatorPeaks)
                {
                    ...
                    var divergence = DivergenceCommon.CheckDivergence(
                        closePrices, 
                        movingAverageOutputs.Select(indicatorSelector).ToList(),
                        pricePeak, 
                        indicatorPeak, 
                        threshold);
