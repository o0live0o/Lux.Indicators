...
namespace Lux.Indicators.DivergenceDetectors
{
    ...
    /// 布林带背离分析器
    /// </summary>
    public static class BollingerBandsDivergenceAnalyzer
    {
        /// <summary>
        /// 查找布林带背离点
        /// </summary>
        /// <param name="closePrices">收盘价序列</param>
        /// <param name="bollingerBandsOutputs">布林带分析结果序列</param>
        /// <param name="indicatorSelector">选择用于背离检测的指标值的函数，默认使用带宽</param>
        ...
        public static List<DivergenceCommon.DivergencePoint> FindDivergences(
            ...
            List<BollingerBandsOutput> bollingerBandsOutputs,
            Func<BollingerBandsOutput, decimal> indicatorSelector = null,
            int lookbackPeriod = 10,
            decimal threshold = 0.1m)
        {
            // 默认使用带宽进行背离检测
            indicatorSelector ??= output => output.BandWidth;
            
            var divergences = new List<DivergenceCommon.DivergencePoint>();
            
            if (closePrices == null || bollingerBandsOutputs == null || 
                closePrices.Count != bollingerBandsOutputs.Count || closePrices.Count < lookbackPeriod)
            {
                return divergences;
            }
            ...
            var indicatorPeaks = DivergenceCommon.FindLocalExtrema(bollingerBandsOutputs.Select(indicatorSelector).ToList(), lookbackPeriod);
            ...
            foreach (var pricePeak in pricePeaks)
            {
                foreach (var indicatorPeak in indicatorPeaks)
                {
                    ...
                    var divergence = DivergenceCommon.CheckDivergence(
                        closePrices, 
                        bollingerBandsOutputs.Select(indicatorSelector).ToList(),
                        pricePeak, 
                        indicatorPeak, 
                        threshold);
